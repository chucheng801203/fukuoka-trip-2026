...
- 新格式備份（ver 2）以「單字｜讀音」穩定鍵記錄進度，並附資料簽章 `sig`；
  未來字庫插入、刪除或重排都不會錯置。
- 舊格式（純數字索引）備份仍可匯入：建置程序維護 `data/legacy_order.json`
  （凍結的舊版單字順序，**請保留在專案中、不要重新產生**）。
  每次建置會比對目前字庫與快照——順序一致時數字索引直接沿用；
  一旦重排，會自動把「舊索引→新索引」對應表注入頁面，舊備份仍正確對應。
- 數字備份若帶有與目前字庫不同的 `sig`，匯入前會要求使用者確認。
